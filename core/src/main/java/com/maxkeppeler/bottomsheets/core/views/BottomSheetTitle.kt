...
@JvmOverloads constructor(
    ...
    init {
        ...
        val a = ctx.obtainStyledAttributes(attrs, R.styleable.BottomSheetTitle, styleAttr, 0)

        val colorDefault = colorOfAttrs(ctx, R.attr.bottomSheetPrimaryColor, R.attr.colorPrimary)
        val color = a.getColor(R.styleable.BottomSheetTitle_bottomSheetTitleColor, colorDefault)
        setTextColor(color)

        val height = a.getDimensionPixelSize(R.styleable.BottomSheetTitle_bottomSheetTitleLineHeight, 0)
        height.takeIf { it != 0 }?.let { lineHeight = height }

        val fontResId = a.getResourceId(R.styleable.BottomSheetTitle_bottomSheetTitleFont, 0)
        fontResId.takeIf { it != 0 }?.let { typeface = ResourcesCompat.getFont(ctx, it) }

        val spacing =
            a.getFloat(R.styleable.BottomSheetTitle_bottomSheetTitleLetterSpacing, 0f)
